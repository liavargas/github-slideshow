...
def send_report(api_key: str, recipient: str, sender: str, listings: List[Dict]) -> None:
    today = date.today().strftime("%B %d, %Y")
    message = Mail(
        from_email=sender,
        to_emails=recipient,
        subject=f"Daily Home Report — Morris County NJ — {today}",
        html_content=_build_html(today, listings),
        plain_text_content=_build_text(today, listings),
    )
    sg = SendGridAPIClient(api_key)
    response = sg.send(message)
    print(f"Email sent — status {response.status_code} to {recipient}")
...
def _build_html(today: str, listings: List[Dict]) -> str:
    ...
    <body style="font-family:Arial,sans-serif;max-width:680px;margin:0 auto;
      ...
      <div style="background:{GREEN};color:white;padding:24px 28px;border-radius:10px;
        ...
        <p style="margin:6px 0 0;opacity:0.85;font-size:14px">{today} · Morris County, NJ</p>
      </div>
      ...
      <div style="background:#eaf4ea;border:1px solid #c3dfc3;border-radius:8px;
        ...
        · <strong>{top_count}</strong> high-match (4–5 stars)
        <div style="margin-top:6px;color:#555;font-size:12px">
          Criteria: 3–4 bed · 2–3 bath · $750K–$900K · Open floor plan
          · Not on main road · Lower taxes
        </div>
      </div>
      ...
      <div style="margin-top:36px;padding-top:14px;border-top:1px solid #eee;
        ...
        and realtor.com via Google Search. Scores are AI estimates — always verify details
